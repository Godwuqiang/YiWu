...
@interface PharmacyDetailVC ()<UITableViewDataSource,UITableViewDelegate,MKMapViewDelegate>{
      MKMapView  *MapView;
}

@end

@implementation PharmacyDetailVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"药店详情";
    HIDE_BACK_TITLE;
    [self initData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([Util IsStringNil:self.bean.jd] || [Util IsStringNil:self.bean.wd]) {  // 没有经纬度
        ...
        if (indexPath.section==0) {
            ...
            static NSString  *PharmacyMapCellID = @"PharmacyMapCellID";
            UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:PharmacyMapCellID];
            if ( nil == cell ){
                cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:PharmacyMapCellID];
            }
...
-(MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id)annotation{
    ...
    if ([annotation isKindOfClass:[KCAnnotation class]]) {
        static NSString *key1=@"AnnotationKey2";
