...
//    UIWindow *currentWindows = [UIApplication sharedApplication].keyWindow;
    ...
    UIWindow *currentWindows = (UIWindow *)[windows lastObject];
    
    //iOS11新增了_UIInteractiveHighlightEffectWindow，这里要获取的UITextEffectsWindow不是最后的一个object
    for (NSObject * window in windows) {
        if([NSStringFromClass([window class]) isEqualToString:@"UITextEffectsWindow"]){
            currentWindows = (UIWindow *)window;
        }
    }
    
    self.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.2];
    [currentWindows addSubview:self];
    NSLog(@"currentWindows--%@",currentWindows);
