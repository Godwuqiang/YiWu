...
@interface MedicalListVC ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,ServiceBLDelegate>
...
#pragma mark - 初始化数据
- (void)initData{
    self.dataList = [[NSMutableArray array]init];
...
-(void)loadData{
    [self showLoadingUI];
    [self.serviceBL  queryYaoPinListWith:self.seachKey andpageNo:self.pageNo andpageSize:MAX_PAGE_COUNT andTradeCode:@"30012"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 获取模型数据
    DrugBean *bean = self.dataList[indexPath.row];
    // Configure the cell...
    if (nil==bean) {
        cell.textLabel.text = @"暂无数据";
    }else{
        cell.textLabel.text = nil==bean.ypmc?@"无":bean.ypmc;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DrugBean *bean = self.dataList[indexPath.row];
    MedicalDetailTVC *VC = [self.storyboard instantiateViewControllerWithIdentifier:@"MedicalDetailTVC"];
