...
// UIWebViewDelegate,TSWebViewDelegate,JS_JFViewController,

@interface JFH5VC ()<WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler>
...
@implementation JFH5VC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"就诊缴费";
    HIDE_BACK_TITLE;
...
- (void)setUpH5{
    ...
    NSString *url = [NSString stringWithFormat:@"%@goPay.html?access_token=%@&device_type=2&pageNo=1&pageSize=3",H5_HOST,[CoreArchive strForKey:LOGIN_ACCESS_TOKEN]];
    DMLog(@"url=%@",url);
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    NSLog(@"H5调用APP方法名称：%@",message.name);
    DMLog(@"%@",message.body);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        @try {
            [[YKFPay shareYKFPay] obtainIdCode:dict callBack:^(NSDictionary *resultDic) {
                DMLog(@"jzCode resultDic:%@", resultDic);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        @try {
            [[YKFPay shareYKFPay] obtainAuthToken:dict callBack:^(NSDictionary *resultDic) {
                DMLog(@"jzToken resultDic:%@", resultDic);
                NSDictionary  *data = [resultDic objectForKey:@"data"];
