...
@interface DTFVisiterFamilyVC ()<HSAddressPickerVCDelegate, HSDatePickerVCDelegate,UITextViewDelegate,UITextFieldDelegate>
...
-(void)setIsFromReregist:(BOOL)isFromReregist{
    ...
    if(_isFromReregist){
        ...
        if(self.hospitalList.count == 0){
            ...
            self.selectedHospital.text = [NSString stringWithFormat:@"请选择探亲安置医院"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    }else if (indexPath.row ==4){//选择探亲原因
        DMLog(@"选择探亲原因");
        
        
    }else if (indexPath.row == 7){//选择结束时间
        DMLog(@"选择结束事件");
...
//        [self.parentVC presentViewController:vc animated:YES completion:nil];
        ...
        NSString *MinDate = [self dateStringAfterlocalDateForYear:0 Month:3 Day:0 Hour:0 Minute:0 Second:0];
    ...
    }else if (indexPath.row == 9){//选择探亲安置医院
        DMLog(@"选择安置医院");
        if (self.registModel.provice_code == nil) {
            [MBProgressHUD showError:@"请选择探亲地点"];
            return;
        }
...
-(BOOL)canGotoNext{
    ...
    self.registModel.type = @"02";
    self.registModel.reason = @"021";
    ...
    if(self.registModel.address == nil){
        [MBProgressHUD showError:@"请输入探亲地的详细地址用于邮寄登记表与异地就医证历本"];
        return NO;
    ...
    if(self.registModel.end_time ==nil){
        [MBProgressHUD showError:@"请选择3个月以后的日期"];
        return NO;
    }
