...
@interface DXBAddMaterialVC ()<LDImagePickerDelegate>
...
@implementation DXBAddMaterialVC
...
- (void)viewDidLoad {
    ...
    self.label.layer.borderColor = [UIColor colorWithHex:0x7ea4b0].CGColor;
    
    self.bgView.hidden = YES;
    self.bgView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.4];
    
    if ([self.registModel.type isEqualToString:@"01"]) {
        // 异地安置登记
        self.attentionLabel.text = @"1、保证拍摄的安置人姓名清晰可见\n2、拍摄的内容必须为所上传的安置人姓名，其他人无效\n3、拍摄后需要审核后才有效";
        self.label.text = @"安置人户口本照片";
        
    }else {
        self.attentionLabel.text = @"1、保证拍摄的亲属姓名清晰可见\n2、拍摄的内容必须为所上传的亲属姓名，其他人无效\n3、拍摄后需要审核后才有效";
        
        self.label.text = @"直系亲属户口本照片";
    }
    
    
    if([self.registModel.reason isEqualToString:@"013"]){
        
        self.tipsLabel.text = @"请上传直系亲属户籍信息页资料";
    }
...
- (IBAction)submissionButtonClick:(id)sender {
    ...
    if ([Util IsStringNil:self.imageDataStr]) {
        ...
        [MBProgressHUD showError:self.noPotoMessage];
        return;
    }
...
#pragma mark - 拍照
- (IBAction)PhotographButtonClick:(id)sender {
...
- (void)imagePicker:(LDImagePicker *)imagePicker didFinished:(UIImage *)editedImage {
    ...
    DMLog(@"完成选择照片...");
    [self.hukouButton setBackgroundImage:editedImage forState:UIControlStateNormal];
