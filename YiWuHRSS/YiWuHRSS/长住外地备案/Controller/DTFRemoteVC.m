...
@interface DTFRemoteVC ()<HSAddressPickerVCDelegate, HSDatePickerVCDelegate,UITextViewDelegate,UITextFieldDelegate>
...
-(void)setupUI{
    ...
    self.addreeDetails.autocorrectionType = UITextAutocorrectionTypeNo;
...
-(void)setIsFromReregist:(BOOL)isFromReregist{
    ...
    if(_isFromReregist){
        ...
        if([self.registModel.reason isEqualToString:@"011"]){
            
            self.reason.text = @"回原籍居住";
        }else if ([self.registModel.reason isEqualToString:@"012"]){
            
            self.reason.text =  @"户籍异地落户居住";
        }else if ([self.registModel.reason isEqualToString:@"013"]){
            
            self.reason.text =  @"随直系亲属异地居住";
        }else if ([self.registModel.reason isEqualToString:@"021"]){
            
            self.reason.text =  @"探亲";
        }else if ([self.registModel.reason isEqualToString:@"022"]){
            
            self.reason.text =  @"出差";
        }else{
            self.reason.text = @"";
        }
        self.reason.textColor = [UIColor colorWithHex:0x333333];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    }else if (indexPath.row ==4){//选择安置原因
        DMLog(@"选选择安置原因");
        DXBresetReasonView *reasonView = [[[NSBundle mainBundle] loadNibNamed:@"DXBresetReasonView" owner:self.parentVC options:nil] firstObject];
        reasonView.reasonType = self.registModel.reason;
        [reasonView setFrame:self.parentVC.view.bounds];
        reasonView.type = ^(NSString *type) {
            NSLog(@"选选择安置原因========: %@", type);
            if([type isEqualToString:@"1"]){
                
                self.registModel.reason = @"011";
                self.reason.text = @"回原籍居住";
                self.reason.textColor = [UIColor colorWithHex:0x333333];
                
            }else if ([type isEqualToString:@"2"]){
                
                self.registModel.reason = @"012";
                self.reason.text = @"户籍异地落户居住";
                self.reason.textColor = [UIColor colorWithHex:0x333333];
                
            }else if([type isEqualToString:@"3"]){
                
                self.registModel.reason = @"013";
                self.reason.text = @"随直系亲属异地居住";
                self.reason.textColor = [UIColor colorWithHex:0x333333];
            }else{
                self.registModel.reason = nil;
                self.reason.text =  @"请选择安置原因";
                self.reason.textColor = [UIColor colorWithHex:0x999999];
            }
        };
        [self.parentVC.navigationController.view addSubview:reasonView];
        
        
    }else if (indexPath.row == 7){//选择结束事件
        DMLog(@"选择结束时间");
...
//        [self.parentVC presentViewController:vc animated:YES completion:nil];
        ...
        NSString *MinDate = [self dateStringAfterlocalDateForYear:2 Month:0 Day:0 Hour:0 Minute:0 Second:0];
    ...
    }else if (indexPath.row == 9){//选择安置医院
        DMLog(@"选择安置医院");
        if (self.registModel.provice_code == nil) {
            [MBProgressHUD showError:@"请选择安置地点"];
            return;
        }
    ...
    }else if (indexPath.row == 12){//登记表和证历本获取方式选择
        DMLog(@"登记表和证历本获取方式选择");
...
-(BOOL)canGotoNext{
    ...
    if(self.registModel.address == nil){
        [MBProgressHUD showError:@"请输入安置地的详细地址用于邮寄登记表与异地就医证历本"];
        return NO;
    ...
    if(self.registModel.reason == nil){
        [MBProgressHUD showError:@"请选择安置原因"];
        return NO;
    }
    ...
    if(self.registModel.end_time ==nil){
        [MBProgressHUD showError:@"请选择2年以后的日期"];
        return NO;
    }
...
//        return NO;
//    }
    DMLog(@"点击下一步时的选中医院=%@",self.registModel.hospital_id);
...
-(void)textViewDidEndEditing:(UITextView *)textView{
    ...
    if(self.addreeDetails.text.length == 0){
        self.addreeDetails.text = @"请输入安置地的详细地址用于邮寄登记表与异地就医证历本";
