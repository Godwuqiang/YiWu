...
@interface LoginVC ()<LoginHttpBLDelegate>{
//    LoginHttpBL         *loginHttpBL;
...
//    loginHttpBL = [LoginHttpBL sharedManager]; arrow_return
//    loginHttpBL.delegate = self;
    _showpass = NO;
    NSString *mobile = [CoreArchive strForKey:LOGIN_APP_MOBILE];
    if (![Util IsStringNil:mobile]) {
        self.InputMobile.text = mobile;
    }
...
-(void)requestLoginSucceed:(UserBean *)bean{
    ...
    if ([kzt isEqualToString:@"1"]) {    // 卡状态正常
        ...
        if ([self.tp isEqualToString:@"1"]) {
            [self.navigationController popToRootViewControllerAnimated:NO];
        }else if ([self.tp isEqualToString:@"2"]) {
...
//    RegistSelectCardVC *valueView = [self.storyboard instantiateViewControllerWithIdentifier:@"RegistSelectCardVC"];
