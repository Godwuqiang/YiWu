...
@interface YLJmixiVC ()<UITableViewDelegate,UITableViewDataSource,ServiceBLDelegate>
...
@implementation YLJmixiVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"养老待遇明细";
    HIDE_BACK_TITLE;
    [self initData];
    [self afn];
}
...
//    NSString *access_token = @"2";
    [self showLoadingUI];
    [self.serviceBL queryYLMXWith:shbzh andmonth:self.time andAccess_Token:access_token andTradeCode:tradecode];
}
...
- (void)setupUI
{
    ...
    ylmxjinfotableview = [[UITableView alloc]initWithFrame:CGRectMake(0, 132, SCREEN_WIDTH, SCREEN_HEIGHT-132) style:UITableViewStyleGrouped];
    [self.view addSubview:ylmxjinfotableview];
    ylmxjinfotableview.delegate = self;
    ylmxjinfotableview.dataSource = self;
    ylmxjinfotableview.separatorStyle = UITableViewCellSeparatorStyleNone;
    ylmxjinfotableview.backgroundColor = [UIColor colorWithHex:0xeeeeee];
    ylmxjinfotableview.tableHeaderView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, ylmxjinfotableview.bounds.size.width, 0.01f)];
    
    [ylmxjinfotableview registerNib:[UINib nibWithNibName:@"MXCell" bundle:nil] forCellReuseIdentifier:@"MXCell"];
    [ylmxjinfotableview registerNib:[UINib nibWithNibName:@"TiShiCell" bundle:nil] forCellReuseIdentifier:@"TiShiCell"];
}
...
{
    if (_hasData) {
        MXCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MXCell"];
        if ( nil == cell ){
            cell = [[MXCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MXCell"];
        }
        [cell ConfigCellWithTXdymxBean:self.dataList[indexPath.row]];
        return cell;
    }else{
