...
//btn创建
    ...
    for (int i = 0; i < titles.count; i++) {
       ...
       // btn.titleLabel.textColor = [UIColor colorWithHex:0x333333];
        [self.MenuScrollView addSubview:btn];
        }
//    CGFloat width = CGRectGetHeight(self.bounds);
//    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - width, 0, width, width)];
...
//            self.selectedBtn.transform = CGAffineTransformMakeScale(1.2, 1.2);
            [self.selectedBtn setFont:[UIFont fontWithName:@"Helvetica-Bold" size:17]];
            self.selectedBtn.titleLabel.textColor = kSelectedColor;
         //   [self.selectedBtn setFontSize:17];
        }
        btn = nil;
        btn1 = nil;
    }
...
//    view.color = [UIColor colorWithHex:0xF8AB26];
    self.line = view;
    ...
    if (self.style == MenuViewStyleFooldHollow || self.style == MenuViewStyleFoold){
        ...
        view.FillColor = [UIColor whiteColor].CGColor;
    }
...
- (void)click:(MenuViewBtn *)btn
{
    ...
    [self.selectedBtn setFont:[UIFont fontWithName:@"Helvetica" size:17]];
    self.selectedBtn.titleLabel.textColor = kNomalColor;
    btn.selected = YES;
...
//    NSString *name = [NSString stringWithFormat:@"scrollViewDidEndDecelerating:%@",self];
