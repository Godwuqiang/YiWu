...
@interface DTFCitizenCardInfoConfirmVC ()

@property (weak, nonatomic) IBOutlet UILabel *nameLabel;            //姓名
@property (weak, nonatomic) IBOutlet UILabel *insureNumberLabel;    //社会保障号
@property (weak, nonatomic) IBOutlet UILabel *cardNumberLabel;      //卡号
@property (weak, nonatomic) IBOutlet UILabel *bankCardNumberLabel;  //银行卡号
@property (weak, nonatomic) IBOutlet UILabel *bankNameLabel;        //银行名称
@property (weak, nonatomic) IBOutlet UIButton *nextButton;          //下一步
...
@implementation DTFCitizenCardInfoConfirmVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"实人认证";
    self.nameLabel.text = [CoreArchive strForKey:LOGIN_NAME];
    self.insureNumberLabel.text = [Util HeadStr:[CoreArchive strForKey:LOGIN_SHBZH] WithNum:0];//截取|前面的字符串
    self.cardNumberLabel.text = [Util HeadStr:[CoreArchive strForKey:LOGIN_CARD_NUM] WithNum:0];//截取|前面的字符串
...
-(void)zhimaAuthFinished{
    ...
    NSString *url = @"";
    if(!self.isFromZhimaRetrievePwd){//不是找回密码的回调
