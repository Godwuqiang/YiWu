             ...
             @try {
                 if (!error) {
                     if (imageSampleBuffer == NULL) {
                         // 没有获取图片
                         [MBProgressHUD showError:@"没有获取图片"];
                         return ;
                     }
                     CMTime time = CMSampleBufferGetPresentationTimeStamp(imageSampleBuffer);
                     if (time.timescale <= 0) {
                         // 拍摄的时候应该有拍摄间隔，如果没有则表示会崩溃
                         [MBProgressHUD showError:@"拍摄间隔太短，无法获取图片"];
                         return ;
                     }
                     NSData *imageData = [AVCaptureStillImageOutput jpegStillImageNSDataRepresentation:imageSampleBuffer];
                     UIImage *image = [[UIImage alloc] initWithData:imageData];
                     ...
                     // 相机初始化错误
                     [MBProgressHUD showError:@"相机初始化错误"];
                 }
